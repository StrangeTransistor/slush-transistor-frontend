...
css.list = function (work, stream)
{
	stream = stream || src(work.buns(list__glob))
...
css.list.watch = function (work)
{
	...
	var stream = watch([ work.buns(list__glob) ])

	stream.pipe(through((vfile, _, next) =>
	{
		console.info('less-list')
		next()
	}))

	return css.list(work, stream)
}
